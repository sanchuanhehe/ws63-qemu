...
- 默认基线 **QEMU v10.0.0**（`scripts/build.sh` 克隆该 tag；从 v9.2.4 升级而来）。**同时维护 v9.2.4**。
...
- **IRQ ≥32（GPIO=33、UART=53…LSADC=72）**：用核内自定义 CSR `LOCIEN0-2`(0xBE0)/`LOCIPD0-2`(0xBE8)/
  ...
  实现核内投递**（见 patch-series `patches/0001-target-riscv-*.patch`）：`CPUArchState` 加 `ws63_locien/locipd`；
...
5. 默认 **v10.0.0**(已从 v9.2.4 升级),**两版本各一套 `patches/<tag>/` 序列并行维护**。升级处理的 API 变化:`sysemu/`→`system/`
   ...
   再往后(v10.1/10.2)target/riscv 又有改动,由 `qtest-matrix.yml` 的 v10.2.3 实验性 cell 作漂移雷达;届时按
   [`patches/README.md`](../patches/README.md) 的流程补 `patches/v10.2.x/`。

...
if [ -f "$PW_ELF" ]; then
    echo "==> bs21 pwm_wdt on -M bs21: expecting WDT counter readback + PWM start (chip-bs21)"
    timeout 5 "$QEMU_BIN" -M bs21 -nographic -serial mon:stdio -kernel "$PW_ELF" >"$TMP/pw.out" 2>/dev/null
...
if [ -f "$DMA_ELF" ]; then
    echo "==> bs21 dma_mem on -M bs21: expecting a mem-to-mem copy (chip-bs21 dma driver)"
    timeout 5 "$QEMU_BIN" -M bs21 -nographic -serial mon:stdio -kernel "$DMA_ELF" >"$TMP/dma.out" 2>/dev/null

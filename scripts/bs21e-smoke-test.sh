#!/usr/bin/env bash
# Smoke-test the minimal BS21E QEMU machine (-M bs21e) against ws63-rs BS2X firmware.
...
#
# This is the milestone-1 acceptance for BS21E: blinky + uart_hello boot end-to-end
# on the bs21e machine. Exit 0 = both pass.
#
# BS21E is register- and memory-identical to BS21E at the M1 level (the whole BS2X
# family shares platform_core.h peripheral bases + chip_core_irq.h IRQ numbers, and
# BS21E's M1 memory map matches BS21E's 160K L2RAM layout). So the SAME bare-metal
# bs2x firmware drives this test — the examples/bs21 binaries built with the
# `chip-bs21` (= bs2x family) HAL. There is deliberately no separate `chip-bs21e`
...
"$QEMU_BIN" -M help 2>/dev/null | grep -q '^bs21e ' || { echo "FATAL: this QEMU has no -M bs21e (rebuild: scripts/build.sh)"; exit 2; }

# ---- uart_hello: serial output ----
UART_ELF="$TARGET_DIR/bs21_uart_hello"
if [ -f "$UART_ELF" ]; then
    echo "==> bs2x uart_hello on -M bs21e: expecting UART banner over UART0 @ 0x52081000"
    timeout 5 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio \
...
if [ -f "$BLINKY_ELF" ]; then
    echo "==> bs2x blinky on -M bs21e: expecting GPIO0 (0x57010000) writes + no illegal-instruction traps"
    timeout 3 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio \
...
if [ -f "$SPI_ELF" ]; then
    echo "==> bs21 spi_loopback on -M bs21e: expecting SPI0 TX->RX loopback (chip-bs21 SPI driver)"
    timeout 5 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio \
...
if [ -f "$GADC_ELF" ]; then
    echo "==> bs21 gadc_read on -M bs21e: expecting a GADC conversion (chip-bs21 gadc driver)"
    timeout 5 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio \
...
if [ -f "$I2C_ELF" ]; then
    echo "==> bs21 i2c_scan on -M bs21e: expecting a bus scan to find I2C0 slave 0x50"
    timeout 6 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio \
...
if [ -f "$HID_ELF" ]; then
    echo "==> bs21 hid_demo on -M bs21e: expecting KEYSCAN key + QDEC count (chip-bs21 drivers)"
    timeout 5 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio \
...
if [ -f "$CRNG_ELF" ]; then
    echo "==> bs21 clock_rng on -M bs21e: expecting RTC counter advance + TRNG randoms (chip-bs21)"
    timeout 5 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio \
...
PW_ELF="$TARGET_DIR/bs21_pwm_wdt"
if [ -f "$PW_ELF" ]; then
    echo "==> bs21 pwm_wdt on -M bs21e: expecting WDT counter readback + PWM start (chip-bs21)"
    timeout 5 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio -kernel "$PW_ELF" >"$TMP/pw.out" 2>/dev/null
...
else echo "==> bs21 pwm_wdt: SKIP"; fi


# ---- dma_mem: BS2X MDMA (v151) memory-to-memory transfer ----
DMA_ELF="$TARGET_DIR/bs21_dma_mem"
if [ -f "$DMA_ELF" ]; then
    echo "==> bs21 dma_mem on -M bs21e: expecting a mem-to-mem copy (chip-bs21 dma driver)"
    timeout 5 "$QEMU_BIN" -M bs21e -nographic -serial mon:stdio -kernel "$DMA_ELF" >"$TMP/dma.out" 2>/dev/null
...
else echo "==> bs21 dma_mem: SKIP"; fi

[ "$fail" -eq 0 ] && echo "BS21E SMOKE TEST: PASS" || echo "BS21E SMOKE TEST: FAIL"
exit "$fail"
